"use strict";
// Renders the full-screen "get back to work" reminder overlay. Deliberately
// self-contained (no dependency on sites.js or the per-site gating in
// content.js) so it can be injected into ANY tab on demand — the popup's
// "Preview" button targets whatever tab is currently active, tracked site or
// not, via the activeTab permission, and calling window.__feedlessShowReminder
// afterward is enough to render it.
(function () {
  if (window.__feedlessShowReminder) return; // already injected in this frame

  // A tab left open across an extension reload/update keeps this script's old
  // instance running with a severed chrome.runtime connection; only the
  // storage read below would throw, so guard it the same way content.js does.
  ...
  window.__feedlessShowReminder = showReminder;
})();
